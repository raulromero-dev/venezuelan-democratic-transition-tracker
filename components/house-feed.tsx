...
export function HouseFeed() {
  const [activeParty, setActiveParty] = useState<PartyFilter>("All")
  ...
  const fetchPosts = useCallback(
    async (isRefresh = false, hours = DEFAULT_LOOKBACK_HOURS) => {
      ...
      try {
        ...
        console.log("[v0] Fetching House feed:", JSON.stringify(requestBody))
        ...
        console.log("[v0] Received House posts:", data.posts?.length || 0, "source:", data.source)
        ...
        console.error("[v0] House feed fetch error:", err)
  ...
  const filteredPosts = posts.filter((p) => {
    const matchesParty = activeParty === "All" || p.party === activeParty
    const matchesRelevance =
      !showOnlyRelevant || (p.relevanceScore !== undefined && p.relevanceScore >= RELEVANCE_THRESHOLD)
    return matchesParty && matchesRelevance
  })
  ...
  const partyFilters: PartyFilter[] = ["All", "Republican", "Democrat"]
    ...
    <div className="space-y-6">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <h2 className="text-2xl font-light tracking-tight text-zinc-900 dark:text-white">House X Feed</h2>
            </div>
          ...
          <div className="flex gap-4 overflow-x-auto pb-1">
            {partyFilters.map((party) => (
              ...
              >
                <span
                  className={activeParty === party ? "skew-x-[6deg] inline-block tracking-wider" : "tracking-wider"}
                >
                  {party.toUpperCase()}
                </span>
              </button>
      ...
      {(isLoading || isRefreshing) && posts.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <span className="text-zinc-500 font-mono text-[10px] tracking-wider">LOADING HOUSE POSTS...</span>
        </div>
      )}
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-shrink-0">
                  <Avatar
                    ...
                    className="w-10 h-10 rounded-none border border-white/20"
                  />
                </div>
      ...
      {!isLoading && filteredPosts.length === 0 && !error && (
        <div className="text-center py-20 space-y-4">
          <p className="text-zinc-500 dark:text-zinc-600 font-mono text-[10px] tracking-wider">
            {showOnlyRelevant ? "NO VENEZUELA-RELEVANT HOUSE POSTS FOUND" : "NO HOUSE POSTS FOUND"}
          </p>
          <button
