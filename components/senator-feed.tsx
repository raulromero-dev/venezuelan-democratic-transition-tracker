  ...
  const fetchPosts = useCallback(
    async (isRefresh = false, hours = DEFAULT_LOOKBACK_HOURS) => {
      ...
      try {
        ...
        console.log("[v0] Fetching Senator feed:", JSON.stringify(requestBody))

        const response = await fetch("/api/senator-feed", {
          method: "POST",
        ...
        console.log("[v0] Received Senator posts:", data.posts?.length || 0, "source:", data.source)
        ...
        console.error("[v0] Senator feed fetch error:", err)
    ...
    <div className="space-y-6">
      ...
      {(isLoading || isRefreshing) && posts.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <span className="text-zinc-500 font-mono text-[10px] tracking-wider">LOADING SENATOR POSTS...</span>
        </div>
      )}
