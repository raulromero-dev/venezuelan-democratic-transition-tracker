...
// Get all Senators with handles
function getSenatorHandles(): { handle: string; member: CongressMember }[] {
  return CONGRESS_MEMBERS.filter((m) => m.chamber === "Senate" && m.officialHandle).map((m) => ({
...
export async function GET(request: Request) {
  ...
  try {
    const dbPosts = await getTweets("senators", { lookbackHours, limit: 200 })
    if (dbPosts.length > 0) {
      console.log("[v0] Returning", dbPosts.length, "Senator posts from database")
...
export async function POST(request: Request) {
  console.log("[v0] === Senator Feed API (X API) Started ===")
  ...
  try {
    ...
    const senatorHandles = getSenatorHandles()
    console.log("[v0] Total Senators with handles:", senatorHandles.length)

    // Create a map for quick lookup
    const handleToMember = new Map<string, CongressMember>()
    for (const { handle, member } of senatorHandles) {
      handleToMember.set(handle.toLowerCase(), member)
    }

    const allHandles = senatorHandles.map((s) => s.handle)
    ...
    const postsWithoutRelevance: Omit<ProcessedPost, "relevanceScore" | "relevanceMethod">[] = allTweets.map(
      (tweet) => {
        ...
        return {
          ...
          role: `Senator (${member?.party?.charAt(0) || "?"}-${member?.state || "??"})`,
          content: tweet.text,
          timestamp,
    ...
    try {
      ...
      await updateFeedMetadata("senators", newestId, lookbackHours)
      console.log("[v0] Saved", dbRecords.length, "Senator tweets to database")
    } catch (dbError) {
    ...
    console.error("[v0] === Senator Feed API ERROR ===")
